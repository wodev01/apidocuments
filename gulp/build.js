...
gulp.task('html', ['inject', 'partials'], function () {
    var partialsInjectFile = gulp.src(path.join(conf.paths.tmp, '/partials/templateCacheHtml.js'), {read: true});
...
gulp.task('carglyApi', function () {
    var fileFilter = $.filter(function (file) {
        return file.stat.isFile();
    });

    return gulp.src([
            path.join(conf.paths.src, '/cargly-api/*')
        ])
        .pipe(gulp.dest(path.join(conf.paths.dist, 'cargly-api/')));
});
...
gulp.task('build', ['html', 'fonts', 'other', 'faFonts', 'carglyApi']);
